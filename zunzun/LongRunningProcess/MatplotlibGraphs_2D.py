...
def DetermineScientificNotationFromString(inData, in_String):
    ...
    if tempString == "ON":
        ...
        if (maxVal > 100.0) or (minVal < -100.0) or (deltaVal < 0.05):
            scientificNotation = True
    return scientificNotation


def CommonPlottingCode(
    in_WidthInPixels,
    in_HeightInPixels,
    in_XName,
    in_YName,
    ...
    fig = plt.figure(
        figsize=(float(in_WidthInPixels) / 100.0, float(in_HeightInPixels) / 100.0), dpi=100
    )
...
def HistogramPlot_NoDataObject(
    in_DataToPlot,
    in_FileNameAndPath,
    in_DataName,
    in_FillColor,
    ...
    scientificNotation = DetermineScientificNotationFromString(
        in_DataToPlot, in_UseScientificNotationIfNeeded
    )
...
def ScatterPlotWithOptionalModel_NoDataObject(
    ...
    if in_Equation:  # make model data for plotting, clipping at boundaries
        ...
        xRange = numpy.arange(
            lowerXbound,
            upperXbound,
            (upperXbound - lowerXbound) / (20.0 * float(in_WidthInPixels + in_HeightInPixels)),
        )  # make this 'reverse-xy-independent'
        tempDataCache = in_Equation.dataCache

        in_Equation.dataCache = pyeq3.dataCache()
        in_Equation.dataCache.allDataCacheDictionary["IndependentData"] = numpy.array(
            [xRange, xRange]
        )
    ...
    if reverseXY:
        ...
        if (min(in_DataToPlot[0]) < in_GraphBounds[0]) or (
            max(in_DataToPlot[0]) > in_GraphBounds[1]
        ):
            matplotlib.pyplot.ylim(in_GraphBounds[0], in_GraphBounds[1])
        if (min(in_DataToPlot[1]) < in_GraphBounds[2]) or (
            max(in_DataToPlot[1]) > in_GraphBounds[3]
        ):
        ...
        if in_Equation:
            ...
            if inConfidenceIntervalsFlag:
                ...
                confs = t_value * numpy.sqrt(
                    ...
                    * (
                        ...
                        + (
                            ...
                            / (
                                (numpy.sum(numpy.power(in_DataToPlot[0], 2)))
                                - n * (numpy.power(mean_x, 2))
                            )
                        )
                    )
                )
                ...
                loglinplot(
                    xRange[booleanMask], lower[booleanMask], linestyle="solid", color="white"
                )
                loglinplot(
                    xRange[booleanMask], upper[booleanMask], linestyle="solid", color="white"
                )
                loglinplot(
                    xRange[booleanMask], lower[booleanMask], linestyle="dashed", color="blue"
                )
                loglinplot(
                    xRange[booleanMask], upper[booleanMask], linestyle="dashed", color="blue"
                )
...
def ContourPlot_NoDataObject(
    ...
    ax.plot(
        in_DataToPlot[0], in_DataToPlot[1], "o", color="0.8", markersize=3
    )  # now that autoscaling is done, use all data for scatterplot - draw these first so contour lines overlay.  Color=number is grayscale
...
def ContourPlot(in_DataObject, in_FileNameAndPath):
    ...
    for i in range(len(X)):
        ...
        in_DataObject.equation.dataCache.allDataCacheDictionary["IndependentData"] = numpy.array(
            [X[i], Y[i]]
        )
...
def HistogramPlot(in_DataObject, in_FileNameAndPath, in_DataName, in_DataToPlot, in_pdfFlag=0):
    ...
    if in_pdfFlag:
        distro = getattr(
            scipy.stats,
            in_DataObject.fittedStatisticalDistributionsList[in_DataObject.distributionIndex][1][
                "distributionName"
            ],
        )  # convert distro name back into a distribution object
        params = in_DataObject.fittedStatisticalDistributionsList[in_DataObject.distributionIndex][
            1
        ]["fittedParameters"]

    HistogramPlot_NoDataObject(
        in_DataToPlot,
        in_FileNameAndPath,
        in_DataName,
        "lightgrey",
...
def ScatterPlot(
    ...
    YAxisData,
    ScientificNotationY,
    UseDataObjectGraphRangeOrCalculate,
    Range1,
    Range2,
    in_LogY,
    in_LogX,
    ...
    if (
        UseDataObjectGraphRangeOrCalculate
    ):  # for data graphs scale with user-supplied values.  For error graphs calculate
        if Range1 == "X":
            gxmin = in_DataObject.gxmin
            gxmax = in_DataObject.gxmax
        if Range1 == "Y":
    ...
    ScatterPlotWithOptionalModel_NoDataObject(
        [XAxisData, YAxisData],
        FileName,
        XAxisName,
        YAxisName,
        ...
        "UseOffset_ON",
        "reverseXY_OFF",
        "XScientificNotation_" + ScientificNotationX,
        "YScientificNotation_" + ScientificNotationY,
...
def ModelAndScatterPlot(
    ...
    ScatterPlotWithOptionalModel_NoDataObject(
        [in_DataObject.IndependentDataArray[0], in_DataObject.DependentDataArray],
        FileName,
        XAxisName,
        YAxisName,
        in_DataObject.graphWidth,
        ...
        "UseOffset_ON",
        reverseXY_string,
        "XScientificNotation_" + in_DataObject.ScientificNotationX,
        "YScientificNotation_" + in_DataObject.ScientificNotationY,
        [gxmin, gxmax, gymin, gymax],
        in_LogY,
        in_LogX,
        in_DataObject.pngOnlyFlag,
