...
class CodeReportFORTRAN90(TextOnlyReport):
    ...
    def CreateReportOutput(self):
        code = pyeq3.outputSourceCodeService().GetOutputSourceCodeFORTRAN90(
            self.dataObject.equation
        )
        self.stringList.append(code)
...
class CodeReportJAVASCRIPT(TextOnlyReport):
    ...
    def CreateReportOutput(self):
        code = pyeq3.outputSourceCodeService().GetOutputSourceCodeJAVASCRIPT(
            self.dataObject.equation
        )
        self.stringList.append(code)
...
class CoefficientListing(TextOnlyReport):
    ...
    def CreateReportOutput(self):
        ...
        if self.dataObject.equation.splineFlag:
            ...
            fittingTargetText = (
                "Fitting target of lowest "
                + self.dataObject.equation.fittingTargetDictionary[
                    self.dataObject.equation.fittingTarget
                ]
            )
            self.stringList.append(
                ...
                + " = %.16E"
                % (
                    self.dataObject.equation.CalculateAllDataFittingTarget(
                        self.dataObject.equation.solvedCoefficients
                    )
                )
                + "\n"
            )
...
class CoefficientAndFitStatistics(TextOnlyReport):
    ...
    def CreateReportOutput(self):
        ...
        self.stringList.append(
            "Degrees of freedom (error):       " + str(self.dataObject.equation.df_e)
        )
        self.stringList.append(
            "Degrees of freedom (regression):  " + str(self.dataObject.equation.df_r)
        )
        ...
        if self.dataObject.equation.sumOfSquaredErrors == None:
            ...
            self.stringList.append(
                "Chi-squared:                      "
                + str(self.dataObject.equation.sumOfSquaredErrors)
            )
        ...
        if self.dataObject.equation.r2 == None:
            ...
            self.stringList.append(
                "R-squared:                        " + str(self.dataObject.equation.r2)
            )
        ...
        if self.dataObject.equation.r2adj == None:
            ...
            self.stringList.append(
                "R-squared adjusted:               " + str(self.dataObject.equation.r2adj)
            )
        ...
        if self.dataObject.equation.Fstat == None:
            ...
            self.stringList.append(
                "Model F-statistic:                " + str(self.dataObject.equation.Fstat)
            )
        ...
        if self.dataObject.equation.Fpv == None:
            ...
            self.stringList.append(
                "Model F-statistic p-value:        " + str(self.dataObject.equation.Fpv)
            )
        ...
        if self.dataObject.equation.ll == None:
            ...
            self.stringList.append(
                "Model log-likelihood:             " + str(self.dataObject.equation.ll)
            )
        ...
        if self.dataObject.equation.aic == None:
            ...
            self.stringList.append(
                "AIC:                              " + str(self.dataObject.equation.aic)
            )
        ...
        if self.dataObject.equation.bic == None:
            ...
            self.stringList.append(
                "BIC:                              " + str(self.dataObject.equation.bic)
            )
        ...
        if self.dataObject.equation.rmse == None:
            ...
            self.stringList.append(
                "Root Mean Squared Error (RMSE):   " + str(self.dataObject.equation.rmse)
            )
        ...
        for i in range(len(self.dataObject.equation.solvedCoefficients)):
            ...
            self.stringList.append(
                "\n%s = %-.16E"
                % (
                    self.dataObject.equation.GetCoefficientDesignators()[i],
                    self.dataObject.equation.solvedCoefficients[i],
                )
            )
            try:
                self.stringList.append(
                    "&nbsp;       std err:                  %-.5E"
                    % (self.dataObject.equation.sd_beta[i])
                )
            except:
            ...
            self.stringList.append(
                ...
                + "[%-.5E, %-.5E]"
                % (self.dataObject.equation.ci[i][0], self.dataObject.equation.ci[i][1])
            )
    ...
    def CreateReportOutput(self):
        ...
        for i in reversed(list(range(dopIndep1))):
            ...
            for j in range(datalen):  # number of data points
                datapoint = self.dataObject.equation.dataCache.allDataCacheDictionary[
                    "IndependentData"
                ][0][j]
                if float(("% ." + str(i + 1) + "E") % (datapoint)) != float(
                    ("% ." + str(i) + "E") % (datapoint)
                ):
                    dopIndep1 = i + 1
                    breakLoop = True
                ...
                for i in range(datalen):  # number of data points
                    ...
                    self.stringList.append(
                        ...
                        % (
                            ...
                            self.dataObject.equation.modelPredictions[i],
                            self.dataObject.equation.modelAbsoluteError[i],
                        )
                    )
            else:
                for i in range(datalen):  # number of data points
                    ...
                    self.stringList.append(
                        ...
                        % (
                            ...
                            self.dataObject.equation.modelPredictions[i],
                            self.dataObject.equation.modelAbsoluteError[i],
                            self.dataObject.equation.modelRelativeError[i],
                        )
                    )
        else:
            self.stringList.append(
                "&nbsp;Indep. Data 1     Indep. Data 2    Dependent Data       Predicted         Abs Error       Rel Error <td></tr>"
            )
            self.stringList.append("")
            if str(self.dataObject.equation.modelRelativeError) == "None":
                for i in range(datalen):  # number of data points
...
class StatisticsListing(TextOnlyReport):
    ...
    def CreateReportOutput(self):
        if self.dataObject.equation.dataCache.DependentDataContainsZeroFlag == 0:
            ...
            self.stringList.append(
                "NOTE: Relative error statistics cannot be compiled, as at least one of"
            )
            self.stringList.append(
                "the dependent variable data points contains a value of exactly zero.\n"
            )
...
class StatisticalDistributions(TextOnlyReport):
    ...
    def CreateCharacterizerOutput(self):
        ...
        for i in self.dataObject.fittedStatisticalDistributionsList:
            ...
            self.stringList.append(
                "Fit Statistics for "
                + str(len(self.dataObject.IndependentDataArray[0]))
                + " data points:<br>"
            )
            self.stringList.append(
                "&nbsp;   " + "Negative Two Log Likelihood = %-.16E<br>" % (2.0 * i[1]["nnlf"])
            )
            ...
            if numpy.isfinite(i[1]["AICc_BA"]):
                self.stringList.append(
                    "&nbsp;   " + "AICc (Burnham and Anderson) = %-.16E<br>" % (i[1]["AICc_BA"])
                )
            else:
            ...
            for parmIndex in range(len(i[1]["parameterNames"])):
                self.stringList.append(
                    "&nbsp;   "
                    + i[1]["parameterNames"][parmIndex]
                    + " = %-.16E" % (i[1]["fittedParameters"][parmIndex])
                )
...
class AbsoluteErrorHistogram(GraphReport):
    ...
    def CreateReportOutput(self):
        MatplotlibGraphs_2D.HistogramPlot(
            self.dataObject,
            self.physicalFileLocation,
            "Absolute Error",
            self.dataObject.equation.modelAbsoluteError,
        )
...
class RelativeErrorHistogram(GraphReport):
    ...
    def CreateReportOutput(self):
        MatplotlibGraphs_2D.HistogramPlot(
            ...
            self.physicalFileLocation,
            "Relative Error",
            self.dataObject.equation.modelRelativeError,
        )
...
class PercentErrorHistogram(GraphReport):
    ...
    def CreateReportOutput(self):
        MatplotlibGraphs_2D.HistogramPlot(
            ...
            self.physicalFileLocation,
            "Percent Error",
            self.dataObject.equation.modelPercentError,
        )
...
class Data2Histogram(GraphReport):
    ...
    def CreateCharacterizerOutput(self):
        MatplotlibGraphs_2D.HistogramPlot(
            ...
            self.dataObject.IndependentDataArray[1],
        )
...
class DependentDataHistogram(GraphReport):
    ...
    def CreateCharacterizerOutput(self):
        MatplotlibGraphs_2D.HistogramPlot(
            ...
            self.dataObject.DependentDataArray,
        )
...
class AbsoluteErrorVsDependentData_ScatterPlot(GraphReport):
    ...
    def CreateReportOutput(self):
        MatplotlibGraphs_2D.ScatterPlot(
            self.dataObject,
            self.physicalFileLocation,
            self.dataObject.DependentDataName,
            self.dataObject.DependentDataArray,
            self.ScientificNotationXAxis,
            "Absolute Error",
            self.dataObject.equation.modelAbsoluteError,
            "AUTO",
            0,
...
class AbsoluteErrorVsIndependentData1_ScatterPlot(GraphReport):
    ...
    def CreateReportOutput(self):
        MatplotlibGraphs_2D.ScatterPlot(
            ...
            self.dataObject.IndependentDataName1,
            self.dataObject.IndependentDataArray[0],
            self.dataObject.ScientificNotationX,
            "Absolute Error",
            self.dataObject.equation.modelAbsoluteError,
            "AUTO",
            0,
...
class AbsoluteErrorVsIndependentData2_ScatterPlot(GraphReport):
    ...
    def CreateReportOutput(self):
        MatplotlibGraphs_2D.ScatterPlot(
            ...
            self.dataObject.IndependentDataName2,
            self.dataObject.IndependentDataArray[1],
            self.dataObject.ScientificNotationY,
            "Absolute Error",
            self.dataObject.equation.modelAbsoluteError,
            "AUTO",
            0,
...
class DependentDataVsIndependentData1_ScatterPlot(GraphReport):
    ...
    def PrepareForCharacterizerOutput(self):
        if self.dataObject.dimensionality == 1:
            return
        self.name = (
            self.dataObject.DependentDataName + " vs. " + self.dataObject.IndependentDataName1
        )
...
class DependentDataVsIndependentData1_ModelPlot(GraphReport):
    ...
    def CreateReportOutput(self):
        MatplotlibGraphs_2D.ModelAndScatterPlot(
            self.dataObject,
            self.physicalFileLocation,
            self.dataObject.IndependentDataName1,
            self.dataObject.DependentDataName,
...
class DependentDataVsIndependentData2_ScatterPlot(GraphReport):
    ...
    def PrepareForCharacterizerOutput(self):
        if self.dataObject.dimensionality != 3:
            return
        self.name = (
            self.dataObject.DependentDataName + " vs. " + self.dataObject.IndependentDataName2
        )
    ...
    def CreateCharacterizerOutput(self):
        MatplotlibGraphs_2D.ScatterPlot(
            ...
            self.dataObject.DependentDataName,
            self.dataObject.DependentDataArray,
            self.dataObject.ScientificNotationZ,
            1,
            "Y",
            "Z",
            self.dataObject.logLinZ,
            self.dataObject.logLinY,
        )
...
class IndependentData1VsDependentData_ScatterPlot(GraphReport):
    ...
    def PrepareForCharacterizerOutput(self):
        if self.dataObject.dimensionality == 1:
            return
        self.name = (
            self.dataObject.IndependentDataName1 + " vs. " + self.dataObject.DependentDataName
        )
...
class IndependentData1VsIndependentData2_ScatterPlot(GraphReport):
    ...
    def PrepareForCharacterizerOutput(self):
        if self.dataObject.dimensionality != 3:
            return
        self.name = (
            self.dataObject.IndependentDataName1 + " vs. " + self.dataObject.IndependentDataName2
        )
...
class IndependentData2VsDependentData_ScatterPlot(GraphReport):
    ...
    def PrepareForCharacterizerOutput(self):
        if self.dataObject.dimensionality != 3:
            return
        self.name = (
            self.dataObject.IndependentDataName2 + " vs. " + self.dataObject.DependentDataName
        )
...
class IndependentData2VsIndependentData1_ScatterPlot(GraphReport):
    ...
    def PrepareForCharacterizerOutput(self):
        if self.dataObject.dimensionality != 3:
            return
        self.name = (
            self.dataObject.IndependentDataName2 + " vs. " + self.dataObject.IndependentDataName1
        )
...
def CharacterizerReportsDict(dataObject):
    return {
        "Text Reports": [CharacterizerStatisticsListing(dataObject)],
        "Graph Reports": [
            Data1Histogram(dataObject),
...
def FittingReportsDict(dataObject):
    return {
        "Text Reports": [
            UserDefinedFunctionText(dataObject),
