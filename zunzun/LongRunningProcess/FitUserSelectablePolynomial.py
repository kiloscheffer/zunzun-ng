...
class FitUserSelectablePolynomial(FittingBaseClass.FittingBaseClass):
    ...
    def SpecificEquationBoundInterfaceCode(self, request):
        if self.dimensionality == 2:
            ...
            if "polynomialOrderX2D" in request.POST:
                ...
                if self.boundForm.equation.xPolynomialOrder > (
                    len(zunzun.formConstants.polynomialOrder2DChoices) - 1
                ):
                    self.boundForm.equation.xPolynomialOrder = (
                        len(zunzun.formConstants.polynomialOrder2DChoices) - 1
                    )
        else:  # 3D
            ...
            if "polynomialOrderY3D" in request.POST:
                ...
                if self.boundForm.equation.yPolynomialOrder > (
                    len(zunzun.formConstants.polynomialOrder3DChoices) - 1
                ):
                    self.boundForm.equation.yPolynomialOrder = (
                        len(zunzun.formConstants.polynomialOrder3DChoices) - 1
                    )
    ...
    def SpecificEquationUnboundInterfaceCode(self, request):
        if self.rank:
            ...
            if self.dimensionality == 2:
                self.unboundForm.fields["polynomialOrderX2D"].initial = str(
                    self.equation.xPolynomialOrder
                )
            else:
                self.equation.yPolynomialOrder = self.functionFinderResultsList[self.rank - 1][7]
                self.unboundForm.fields["polynomialOrderX3D"].initial = str(
                    self.equation.xPolynomialOrder
                )
                self.unboundForm.fields["polynomialOrderY3D"].initial = str(
                    self.equation.yPolynomialOrder
                )
