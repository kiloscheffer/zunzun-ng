...
2. The shareable result pages (`equation_fit_or_characterizer_results.html`, `function_finder_results.html`) now DO carry the off-request globals — the child injects them via `StatusMonitoredLongRunningProcessPage._inject_offrequest_globals` before `render_to_string` (which, unlike `render(request, ...)`, skips context processors). As of 2026-06-15 that helper injects two globals: `demo_mode` (the watermark) and `head_html` (the operator `<head>` markup; see `docs/superpowers/specs/2026-06-15-head-html-injection-design.md`). What still lacks *both*: the off-request *error/fallback* pages the child writes — `exception_while_fitting_an_equation.html` (a standalone template that does not even extend the base, so it has no injection point) and the three `generic_error.html` child renders (`StatusMonitoredLongRunningProcessPage.py:734` & `:1160`, `child_payload.py:195`), which `extends` the base but are handed bare dicts. On those rare pages neither the watermark nor the operator `HEAD_HTML` appears.

**Hypothesis / impact.** (1) is bounded, not a fork-bomb: the pre-existing concurrency gate (`MAX_CONCURRENT_FITS_PER_IP`) already caps *simultaneous* render children, so the only unbounded dimension is sustained hourly CPU, not instantaneous load. (2) affects only rare error/fallback pages — the normal shareable result links carry both globals; the watermark gap there is cosmetic and the `HEAD_HTML` gap drops the operator's `<head>` markup (e.g. analytics) only on those infrequent error renders.

**What we didn't do.** Deliberately kept "one POST == one run" (the approved spec semantics) so legitimate result-browsing doesn't burn the fit quota and produce a mid-browse 429. For (2), wired the off-request globals into the two success-result render sites only; left the error/fallback render sites unwired (rare paths, low value) — they can reuse the same `_inject_offrequest_globals` helper if desired. The `head_html` injection (2026-06-15) inherited this exact boundary rather than widening it.

**Where to pick up.** (1) If sustained hourly CPU abuse becomes real, either count `FunctionFinderResults/` GETs in the demo guard (mirror the `_will_spawn_child` predicate in `views.py`) or add a separate, looser hourly cap for that path. (2) Route the error/fallback render sites through `_inject_offrequest_globals` too — pass `self._inject_offrequest_globals({"error": ...})` at `StatusMonitoredLongRunningProcessPage.py:734` & `:1160` (and the equivalent in `child_payload.py:195`, with care: that one is a last-ditch render fired when the template loader itself may be broken). One change then closes the gap for both `demo_mode` and `head_html`.
