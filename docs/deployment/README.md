...
ZunZunNG enforces two concurrency caps: `MAX_CONCURRENT_FITS_PER_SESSION` (prod default 1, env override `ZUNZUN_MAX_CONCURRENT_FITS_PER_SESSION`) and `MAX_CONCURRENT_FITS_PER_IP` (prod default 4, env override `ZUNZUN_MAX_CONCURRENT_FITS_PER_IP`). Defaults are DEBUG-aware: under `runserver` (dev) both relax to 10 so local multi-fit work isn't walled; under Waitress/WSGI (prod) they are 1 and 4. These settings live in `settings.py` and can be tuned without code changes — a personal-production box (Waitress, single user) that wants the relaxed posture sets the env vars.
