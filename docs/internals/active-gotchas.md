...
- Rate limiting is always in effect (no install-time gating). To disable for local testing, set `RATELIMIT_ENABLE = False` in `settings.py`. When a caller exceeds the rate, `CommonToAllViewsMiddleware` applies a 5-second `time.sleep` after the view returns — this specifically breaks polling endpoints (`StatusUpdateView` is intentionally not decorated with `@ratelimit` for this reason).
