...
- `.pre-commit-config.yaml` provides optional local hooks (`commit-check` for message + branch, `mypy` for type checking). Opt-in per developer via `uv tool install pre-commit && pre-commit install --hook-type commit-msg --hook-type pre-push`. CI re-runs the equivalent via `commit-check.yml` and `ci.yml`, so the local install is a convenience, not a merge gate.
